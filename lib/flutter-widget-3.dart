...
Map<String, WidgetBuilder> getRouteMap() {
  ...
  map.putIfAbsent(
      pages[0], () => (BuildContext ctx) => RowColumnSample(title: pages[0]));
  map.putIfAbsent(
      pages[1], () => (BuildContext ctx) => ContainerSample(title: pages[1]));
  map.putIfAbsent(
      pages[2], () => (BuildContext ctx) => GridViewSample(title: pages[2]));
  map.putIfAbsent(
      pages[3], () => (BuildContext ctx) => ListViewSample(title: pages[3]));
  map.putIfAbsent(
      pages[4], () => (BuildContext ctx) => StackSample(title: pages[4]));
  map.putIfAbsent(
      pages[5], () => (BuildContext ctx) => LayoutSample1(title: pages[5]));
  map.putIfAbsent(
      pages[6], () => (BuildContext ctx) => LayoutSample2(title: pages[6]));
  map.putIfAbsent(
      pages[7], () => (BuildContext ctx) => StateSample(title: pages[7]));
  map.putIfAbsent(
      pages[8], () => (BuildContext ctx) => AssetSample(title: pages[8]));
...
class LayoutBuildImpl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Layout build sample"),
      ),
