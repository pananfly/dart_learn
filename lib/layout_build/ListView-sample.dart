...
class ListViewSample extends StatelessWidget {
  ...
  ListViewSample({Key? key, required this.title}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    ListTile _buildListItem(String title, String subTitle, IconData icon) =>
        ListTile(
          title: Text(
            title,
    ...
    Widget _buildList() => ListView(
          children: [
            _buildListItem(
                'CineArts at the Empire', '85 W Portal Ave', Icons.theaters),
            _buildListItem(
                'The Castro Theater', '429 Castro St', Icons.theaters),
            _buildListItem(
                'Alamo Drafthouse Cinema', '2550 Mission St', Icons.theaters),
            _buildListItem('Roxie Theater', '3117 16th St', Icons.theaters),
            _buildListItem('United Artists Stonestown Twin',
                '501 Buckingham Way', Icons.theaters),
            _buildListItem(
                'AMC Metreon 16', '135 4th St #3000', Icons.theaters),
            Divider(),
            _buildListItem(
                'K\'s Kitchen', '757 Monterey Blvd', Icons.restaurant),
            _buildListItem(
                'Emmy\'s Restaurant', '1923 Ocean Ave', Icons.restaurant),
            _buildListItem('Chaiya Thai Restaurant', '272 Claremont Blvd',
                Icons.restaurant),
    ...
    List<ListViewData> _listDatas = [
      ...
      ListViewData('United Artists Stonestown Twin', '501 Buckingham Way',
          Icons.theaters),
    ...
    ListTile _buildListItem2(BuildContext buildContext, int index) => ListTile(
          key: ObjectKey(_listDatas[index]),
          title: Text(
